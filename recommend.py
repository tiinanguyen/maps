...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    location = [restaurant_location(i) for i in cluster]
    return[mean([x[0] for x in location]), mean([x[1] for x in location])]
    # END Question 5
...
def find_predictor(user, restaurants, feature_fn):
    ...
    mean_x = mean(xs)
    mean_y = mean(ys)
    sum_xx = sum([pow(x - mean_x, 2) for x in xs])
    sum_yy = sum([pow(y - mean_y, 2) for y in ys])
    sum_xy = sum([(x-mean_x)*(y-mean_y) for x,y in zip(xs,ys)])

    b, a, r_squared = sum_xy/sum_xx, mean_y-sum_xy/sum_xx*mean_x, pow(sum_xy, 2)/(sum_xx*sum_yy)
    # END Question 7
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
            if restaurant in reviewed:
                rest[restaurant_name(restaurant)] = user_rating(user, restaurant_name(restaurant))
            else:
                rest[restaurant_name(restaurant)] = predictor(restaurant)
    return rest
